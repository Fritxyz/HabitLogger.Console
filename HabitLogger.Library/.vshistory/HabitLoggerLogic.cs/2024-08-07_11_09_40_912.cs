                    ...
                    case 3:
                        ...
                        do
                        {
                            do
                            {
                                ...
                                isQuantityParsed = int.TryParse(idStr, out id);

                                if (!isQuantityParsed || id < 0)
                                    Console.WriteLine("\nInvalid Quantity");


                            } while (!isQuantityParsed || id < 0) ;

                            isDataDeleted = HabitLoggerCrud.DeleteData(id);

                            if (!isDataDeleted) Console.WriteLine("Data doesn't exist. Please try again");
                            else Console.WriteLine("Data deleted");

                        } while (isDataDeleted == false);
                        // TODO: by entering an id, the data will be deleted
